...
class ChecklistRespostaDao extends DatabaseAccessor<AppDatabase>
  ...
  ChecklistRespostaDao(AppDatabase db) : super(db);
  ...
  Future<List<ChecklistRespostaTableDto>> listar() async {
    ...
    return results.map((row) => ChecklistRespostaTableDto.fromEntity(row)).toList();
  }
  ...
  Future<ChecklistRespostaTableDto?> buscarPorId(int id) async {
    ...
    try {
      ...
      if (result == null) {
        AppLogger.w('Resposta não encontrada para ID: $id', tag: 'ChecklistRespostaDao');
        return null;
      }
      
      return ChecklistRespostaTableDto.fromEntity(result);
    } catch (e) {
      AppLogger.e('Erro ao buscar resposta por ID: $e', tag: 'ChecklistRespostaDao');
      return null;
    }
  }
  ...
  Future<List<ChecklistRespostaTableDto>> buscarPorChecklistPreenchido(int checklistPreenchidoId) async {
    AppLogger.d('Buscando respostas por checklist preenchido: $checklistPreenchidoId', tag: 'ChecklistRespostaDao');
  ...
  Future<ChecklistRespostaTableDto?> buscarPorPergunta(int checklistPreenchidoId, int perguntaId) async {
    AppLogger.d('Buscando resposta por pergunta: $perguntaId', tag: 'ChecklistRespostaDao');
    
    try {
      final result = await (select(checklistRespostaTable)
            ..where((r) => r.checklistPreenchidoId.equals(checklistPreenchidoId) & r.perguntaId.equals(perguntaId)))
          .getSingleOrNull();
      
      if (result == null) {
        AppLogger.w('Resposta não encontrada para pergunta: $perguntaId', tag: 'ChecklistRespostaDao');
        return null;
      }
      
      return ChecklistRespostaTableDto.fromEntity(result);
    } catch (e) {
      AppLogger.e('Erro ao buscar resposta por pergunta: $e', tag: 'ChecklistRespostaDao');
      return null;
    }
  }
  ...
  Future<int> inserir(ChecklistRespostaTableDto dto) async {
    AppLogger.d('Inserindo resposta para pergunta: ${dto.perguntaId}', tag: 'ChecklistRespostaDao');
  ...
  Future<List<int>> inserirMultiplos(List<ChecklistRespostaTableDto> dtos) async {
    AppLogger.d('Inserindo ${dtos.length} respostas', tag: 'ChecklistRespostaDao');
    ...
    try {
      ...
      AppLogger.i('${ids.length} respostas inseridas', tag: 'ChecklistRespostaDao');
      return ids;
    } catch (e) {
      AppLogger.e('Erro ao inserir múltiplas respostas: $e', tag: 'ChecklistRespostaDao');
      rethrow;
    }
  }
  ...
  Future<bool> atualizar(ChecklistRespostaTableDto dto) async {
    AppLogger.d('Atualizando resposta ID: ${dto.id}', tag: 'ChecklistRespostaDao');
    ...
    try {
      ...
      AppLogger.e('Erro ao atualizar resposta: $e', tag: 'ChecklistRespostaDao');
      return false;
    }
  }
  ...
  Future<bool> removerPorChecklistPreenchido(int checklistPreenchidoId) async {
    AppLogger.d('Removendo respostas do checklist preenchido: $checklistPreenchidoId', tag: 'ChecklistRespostaDao');
    ...
    try {
      ...
      AppLogger.e('Erro ao remover respostas do checklist: $e', tag: 'ChecklistRespostaDao');
      return false;
    }
  }
  ...
  Future<int> contarPorChecklistPreenchido(int checklistPreenchidoId) async {
    AppLogger.d('Contando respostas por checklist preenchido: $checklistPreenchidoId', tag: 'ChecklistRespostaDao');
    
    final result = await (selectOnly(checklistRespostaTable)
          ..addColumns([checklistRespostaTable.id.count()])
          ..where(checklistRespostaTable.checklistPreenchidoId.equals(checklistPreenchidoId)))
        .getSingle();
  ...
  Future<bool> existeRespostaParaPergunta(int checklistPreenchidoId, int perguntaId) async {
    AppLogger.d('Verificando se existe resposta para pergunta: $perguntaId', tag: 'ChecklistRespostaDao');
